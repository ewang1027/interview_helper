...
| `scripts/` | The gates: secret scan, doc links, doc consistency, reference-solution verification — all four run in CI — plus the local push and hygiene checks, and `test_db.sh`, which runs every database-touching suite against a `_test` database of its own. CI runs on every push to `main` and every pull request, and can be re-run by hand with `gh workflow run CI` |
